...
Pro faktor Ruka zamítáme nulovou hypotézu shodnosti støedních hodnot mezi všemi úrovnìmi (p-val = 0.97).
...
```
Fisherùv LSD test vrací pro promìnnou jména least significant difference rovný 4.6 úrovnì jsou tedy dvì rozdílné skupiny (jejich rozdíl je 5.889).
LSD pro Ruka je roven 5.65, všechny úrovnì jsou ve stejné skupinì.
<!-- zahrnout Kruh nebo ne?? -->
Tukeyho honest significant difference test vrací pro promìnnou Jména p-hodnotu 0.017, nezamítáme tedy hypotézu, že úrovnì mají rùznou støední hodnotu. Tento test na promìnné Ruka vrací p-hodnoty indikující, že všechny úrovnì patøí do stejné skupiny.
Výsledk je tedy shodný s pøedchozími testy.
...
plot(aov_celk)
```
ANOVA vrací pro faktory Jmeno a Kruh p-hodnoty < 0.05, tzn, mají vliv na promìnnou Poèe, faktor Ruka signifikantní není, proto jej vyhodíme z modelu.
Residuals vs. Fitted plot ukazuje nìkolik možných outlierù, které ovlivòují lineární trend u obou koncù køivky.
Bod è.17 je znaènì vychýlený i u Normálního Q-Q plotu, kde máme zcela zøetelný lineární trend.
Scale-Location plot má body víceménì rovnomìrnì rozdìlené a køivka zaujímá horizontální polohu, vypadá to tedy na rovnomìrné rozdìlení reziduí pøes prediktory a homoskedasticitu.
...
```
Ve tøetím bodu jsme nìkolika testy (Fisher LSD, Tukey HSD, paiwise t-test) urèili, že faktory Ruka a Jména mají úrovnì s rùznými støedními hodnotami (pocházejí z rùzných skupin). Jinak to bylo s faktorem Kruh, kde všechny úrovnì patøí do stejné.
Model ANOVA má u faktorù Jmeno a Kruh p-hodnoty < 0.05, je zde signifikantní rozdíl mezi úrovnìmi. Faktor Kruh, má velmi vysokou p =hodnotu, není signifikantní.
Závìry se tedy shodují nezávisle na volbì nástroje.
